...
void offboard_class::init_subscriber()
{
	...
	// 订阅机体坐标系下的目标点
	boody_position_sub = nh.subscribe<geometry_msgs::PoseStamped>
		("/camera/ellipse/body/center", 1, &offboard_class::position_cb, this);
	// 订阅世界坐标系下的目标点
...
// @ret: void
// @birth: created by JTJ on 2023.6.4
inline void offboard_class::flightWayPointGlobal(const int st, const int Nextstep)
{
	global_position.pose.position.z = pose.pose.position.z;
